...
export default function Header() {
  ...
  const mobileDrawer = open && mounted
    ? createPortal(
        <div
          id="mobile-navigation"
          className="fixed inset-0 z-[2147483647] flex h-[100dvh] w-full flex-col overflow-hidden bg-white lg:hidden"
          role="dialog"
          aria-modal="true"
          ...
          <nav className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain bg-white px-5 py-6 sm:px-6" aria-label="Navigasi mobile">
            ...
            <div className="mt-auto pt-6 pb-[max(1.25rem,env(safe-area-inset-bottom))]">
              <Link
                href="/contact"
                onClick={() => setOpen(false)}
                className="inline-flex min-h-12 w-full touch-manipulation items-center justify-center gap-2 rounded-full bg-accent px-5 py-3 text-sm font-semibold text-white"
              >
